...
This prints, among other lines: `context.md: <N> lines (cap 150)` (plus `context.md oversize: <N> KB` if the file is over the 16 KB injection cap); `last log entry: Session <N> [placeholder|real]`; `next session: <N>`; today's commits and modified files; codemap freshness; graph size; up to 5 lint candidates (unlinked concept mentions); dangling links; and unannotated directories.
...
2. **Resolve lint candidates** (up to 5, from `sync-prepare`'s output): for each, either confirm it — add one `uses:: [[concepts/<slug>|<Name>]]` line in `context.md`'s `## Architecture` section, directly above the `Full reference:` line (the same place `graph lint`'s auto-apply writes) — or, if it's not a real relationship, dismiss it:
   ...
   ```
3. **Fix the dangling links** `sync-prepare` listed under `dangling links:` (each line is `<node> → [[<target>]]`). For each: correct the target if it's a typo, create the note it should point at, or delete the link. Leaving them costs you the same report — and the same "run /brain sync" health line — every session. A `[[…]]` written inside backticks or a fenced block is a syntax example and is never reported, so anything listed here is a real broken link.
4. **Update `architecture.md`** — append any architectural discoveries from this session (new patterns, conventions, structural changes) as bullets to the relevant existing section, or a new one. Apply the Concept graph promotion rule (SKILL.md) to each: if it's the kind of fact you'd link to from more than one place, create/update its concept note and link it from the bullet. Start a **new** concept note from the template at `${CLAUDE_PLUGIN_ROOT}/templates/concept.md` — read it, fill in every `{placeholder}`, and write it to `VAULT_ROOT/concepts/<concept-slug>.md`.
5. **Append the log entry.** This one is yours to write in full — the `Changed:` line included; the plugin only fills that in on its own `(pre-compact)` / `(auto-close)` checkpoint entries. Read `sync-prepare`'s `last log entry: Session <N> [placeholder|real]` and `next session: <M>` lines: if `[placeholder]`, replace that entry (`(pre-compact)`/`(auto-close)`) in place using session number `<N>`; if `[real]`, append a new entry using `<M>`:
   ```markdown
