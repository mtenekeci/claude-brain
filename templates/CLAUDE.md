...
- Yes → derive `CONCEPT_SLUG` (lowercase, hyphenated name), then check `{vault-root}/concepts/<CONCEPT_SLUG>.md`:
  - Doesn't exist → create `{vault-root}/concepts/<CONCEPT_SLUG>.md` with frontmatter (`concept`, `type`, `updated`), a 1-3 sentence description body, and a `## Used by` entry linking this project. See the concept notes already in `{vault-root}/concepts/` for the exact format.
