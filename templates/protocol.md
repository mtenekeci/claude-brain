...
Rules:
1. Before any Grep/Glob/Read to find where something lives, run `{BRAIN} graph find <term>` (or use the graph hits injected with your prompt). On a miss: search the code, then add a codemap `## Modules` row or an architecture.md bullet so the next session hits.
