...
### Last run — dogfood (Task 7, real vault, not a sandbox)

- **Date:** 2026-09-08. `--plugin-dir` against the maintainer's real `~/.claude/brain.config` and real vault, `async_regen` at its default `true`. Full report: `.superpowers/sdd/2026-09-08-brain-v2-release/task-7-report.md`.
- **`brain.log` is an ERROR log, not a timing or trace log.** It is written only by `config.log_error`. The `SMOKE3 PTU …` / `Stop k=0 m=0 …` lines sitting in `~/.claude/plugins/data/brain-inline/brain.log` are leftovers from temporary Task-6 instrumentation and are **not** shipped behaviour — do not expect them again. Consequence for future runs: an empty `brain.log` diff means "no hook raised", *not* "the hook never fired", and there are **no per-hook timings there to read**.
- **How to actually time a hook:** invoke the entrypoint directly rather than trying to time `claude -p` (which measures the whole model turn).
  ```bash
  echo '{"session_id":"t","cwd":"<repo>","source":"startup","hook_event_name":"SessionStart"}' \
    | CLAUDE_PLUGIN_ROOT="$REPO" python3 "$REPO/brain/__main__.py" hook SessionStart
  ```
  Measured warm: **0.15–0.16 s** for `claude-brain` (19 KB injected), **0.20–0.31 s** for `liryn` (742 tracked files / 112 Swift, 86 KB injected). Cold runs are bounded by the `timeout: 10` in `hooks.json` — a cold run that produced injection was necessarily under 10 s.
- **Check 3, route 2 (the Stop gate actually blocking) observed live for the first time.** Every earlier run took the compliant route 1. To force it: run the check-3 prompt with `--allowedTools "Bash"` only (no Edit/Write) and add *"Do NOT write to the vault … I will handle the vault myself"*. The model then cannot comply and the block surfaces in the reply:
  > I need permission to update the vault files. The stop hook requires me to update `<…>/vault/projects/smoke/context.md`, but I don't have read/write permission granted. Can you approve vault file access?

  The quoted `{context}` path is the observable proof the `decision: block` payload arrived, since `-p` never prints the raw hook string.
- **`SessionEnd` stdout delivery is a non-question:** `on_session_end` returns `EMPTY`. Its whole contract is the direct `log.md` write, verified live (a commit + no sync produced `## … Session 2 (auto-close)` with a git-enriched `Completed: feat: x`).
- **`PreCompact` stdout delivery remains UNVERIFIED — needs an interactive `/compact`.** `claude -p` cannot trigger compaction; the only relevant flag, `--autocompact`, takes 100k–1M tokens and a `-p` call cannot fill that window. Note that `_ADDITIONAL_CONTEXT_EVENTS = ("PostToolUse",)`, so the T6 `additionalContext` fix does **not** cover `PreCompact`.
- **`PreCompact` will not stack a stub on an unenriched entry.** `vault.last_entry_is_placeholder` treats both `(pre-compact)` and `(auto-close)` heads as placeholders, so a PreCompact straight after an auto-close returns `BRAIN SYNC: a checkpoint already exists in … — enrich its Completed/Decided fields` instead of appending. Intentional; don't file it as a miss.
- **Migration line, verified in a clean sandbox** (a live model may fail to echo it when the injection is large — that happened on `liryn`; check the disk artifacts instead):
  `Brain: migrated smoke to v2 layout (claude-md (original backed up to CLAUDE.md.brain-bak), hooks:1, path, codemap). Run /brain sync once to reconcile concepts.`
- **`.brain/` lives in the vault project dir, not the code repo** (`brain/graph.py`: "so other projects' graphs build without their repo checked out"). Don't look for it next to the source.
- **Data-dir note from Check 10 re-confirmed:** Claude Code sets `CLAUDE_PLUGIN_DATA` to `~/.claude/plugins/data/brain-inline` even in a real non-sandbox session. Only a bare direct hook invocation falls back to the `~/.claude/brain-data/` default.
- **Bugs found:** none requiring a code change. Two items left for a reviewer: `/brain sync` resolved unlinked-concept and stale-Used-by candidates but left `3 dangling links` while still printing `(run /brain sync)`; and the `PreCompact` delivery gap above.
